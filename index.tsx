...
import type { Report as TestReport } from './test';
import { Report as Reportcard } from './ui/report';
...
const Indicator = ({ status }: { status: string }) => {
    let colors = 'bg-yellow-500/50 border-yellow-400';
    let pulse = true;

    if (status === 'Chưa khởi tạo' || status === 'Lỗi khởi tạo') {
        colors = 'bg-gray-500/50 border-gray-400';
        if (status === 'Lỗi khởi tạo') {
            colors = 'bg-red-500/50 border-red-400';
        }
        pulse = false;
    } else if (status === 'Sẵn sàng') {
        colors = 'bg-green-500/50 border-green-400';
        pulse = false;
    }
    ...
    return (
        <div className="font-mono mt-4 px-4 py-2 bg-gray-900/50 backdrop-blur-sm border border-gray-700 rounded-full inline-flex items-center gap-3 shadow-md">
            <span className="relative flex h-3 w-3">
                ...
                <span className={`relative inline-flex rounded-full h-3 w-3 ${colors.split(' ')[0]}`}></span>
            </span>
...
function App() {
    ...
    const Card = ({ children, className = '' }: { children: React.ReactNode, className?: string }) => (
        <div className={`bg-gray-800/30 backdrop-blur-md border border-gray-700/50 rounded-xl shadow-lg transition-all duration-300 hover:border-gray-600/80 hover:shadow-2xl ${className}`}>
            {children}
        </div>
    );
    ...
    const Telemetrycard = ({ report }: { report: Record<string, number> | null }) => {
        ...
        return (
            ...
            <Card className="p-5">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <button onClick={copy} className="bg-gray-700/80 hover:bg-gray-600/80 text-xs text-gray-300 font-bold py-1.5 px-4 rounded-md transition-all duration-200 transform hover:scale-105">
                        {status}
                    </button>
                </div>
                <div className="space-y-2 text-sm font-mono">
                    {Object.entries(report).map(([key, value]) => (
                         <p key={key} className="flex justify-between items-baseline border-b border-gray-700/50 pb-1.5">
    ...
    return (
        <div className="container mx-auto p-6 md:p-8 max-w-7xl">
            <header className="text-center mb-10">
                ...
                <p className="text-xl text-gray-400">Động cơ CSDL In-Memory Dạng cột & Tối ưu hóa Truy vấn</p>
            ...
            <main className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <section className="lg:col-span-1 space-y-6">
                    ...
                    <Telemetrycard report={telemetryreport} />
                    {queryresult && (
                       <div className="fade-in">
                        <Card className="p-5">
                             <div className="flex justify-between items-center mb-3">
                                ...
                                <button onClick={copyquery} className="bg-gray-700/80 hover:bg-gray-600/80 text-xs text-gray-300 font-bold py-1.5 px-4 rounded-md transition-all duration-200 transform hover:scale-105">
                                    {querycopystatus}
                                </button>
                            </div>
                             ...
                             <details className="mt-4 text-xs text-gray-500 cursor-pointer group">
                                <summary className="font-semibold group-hover:text-gray-300 transition">Xem kế hoạch thực thi</summary>
                ...
                <section className="lg:col-span-2">
                    <Card className="p-5 h-full flex flex-col">
                        ...
                        <div className="font-mono text-sm text-gray-400 space-y-2 overflow-y-auto flex-grow bg-gray-900/70 p-4 rounded-md min-h-[400px]">
